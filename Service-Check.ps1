...
$pca = @("CortexClient:::0x16ed000","TroxillClient:::0x1b44000","PhasmaClient:::0x16a4000","VapeV4:::0xbcb000","VapeLite:::0x1709000","AmmitDLC:::0x3a23000","NobiumClient:::0x1c06000","NoRender:::0x82ed000","AvaloneClient:::0x19f2000","AvaloneGreen:::0x140b000","AvaloneBlue:::0x13f9000","BlessedClient:::0x2335000","AnapaV4:::0xe96000","DripLite:::0x192a000","TakkerProxy:::2022/07/08:23:42:38","UsbCleaner:::0x2d3000","Hider:::0x16000","Blast3xStringRemover:::0x5c000","R-WipeCleaner:::0x112000","ResourceHacker:::0x61b000","RevoCleaner:::0xe79000","WiseFolderHider:::0xadf000","HideFolders:::0xe0c000","Nemezida:::0x2199000","OceanBypass:::0xe000","Hider2:::0x88000","WhiteGhostInt:::0x349f000","DpsChanger:::0x54000","StringCleaner:::0x76000","StubbornCleaner:::0x1bf2000","Vanish:::0x15e000","MicoHitBoxes:::0x18be000","MerzlotaCleaner:::0x50000","SizeChanger:::0x2b000","Unicorn:::0xa20000")

# Скачивание утилиты во временную папку
$stringsPath = "$env:TEMP\strings64.exe"
Write-Host "Downloading strings64.exe..." -ForegroundColor Cyan
Invoke-WebRequest -Uri "https://live.sysinternals.com/strings64.exe" -OutFile $stringsPath -UseBasicParsing

# Функция для сканирования сервиса
function Scan-Service {
    param($ServiceName, $DataList, $Type)
    $svc = Get-Service -Name $ServiceName 
    if ($svc -and $svc.Status -eq 'Running') {
        $pid = (Get-WmiObject Win32_Service | Where-Object { $_.Name -eq $ServiceName }).ProcessId
        Write-Host "Service $ServiceName enabled. PID: $pid" -ForegroundColor Cyan
        
        $output = (& $stringsPath -p $pid -nobanner | Out-String) -split "`n"
        
        foreach($out in $output) {
            $out = $out.Trim()
            # Поиск модифицированных расширений
            if ($out -match '^\\device\\harddiskvolume((?!.*\.(exe|dll)).)*\..*$') {
                Write-Host "[$ServiceName] Detected modified extension: $out" -ForegroundColor Yellow
            }
            # Поиск строк из базы
            foreach($entry in $DataList) {
                $parts = $entry -split ":::"
                if ($out.Contains($parts[1])) {
                    Write-Host "[$ServiceName] Detected $($parts[0]) ($out)" -ForegroundColor Red
                }
            }
        }
    } else {
        Write-Host "Service $ServiceName disabled or not found" -ForegroundColor Gray
    }
}

# 1. Сканируем DPS и DiagTrack
Scan-Service "DPS" $dps "DPS"
Scan-Service "DiagTrack" $dps "DiagTrack"

# 2. Сканируем Explorer
$explorerpid = (Get-Process -Name explorer).Id
Write-Host "Scanning Explorer. PID: $explorerpid" -ForegroundColor Cyan
$output = (& $stringsPath -p $explorerpid -nobanner | Out-String) -split "`n"
$expArray = @()
foreach($out in $output) {
    if ($out -match "^file:///.+exe*$") {
        $path = $out.Trim().Replace("file:///", "").Replace("%20", " ")
        if ($path -notin $expArray) {
            if (Test-Path $path) {
                $sig = Get-AuthenticodeSignature $path
                if ($sig.Status -ne 'Valid') {
                    Write-Host "[Explorer] Invalid signature: $path" -ForegroundColor Yellow
                } else {
...
# 3. Сканируем PcaSvc
Scan-Service "PcaSvc" $pca "PCA"

# Очистка
Remove-Item $stringsPath -Force
